...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  const row = await env.DB.prepare(`SELECT l.id FROM stake_moderation_log l JOIN club_stake s ON s.id = l.stake_id JOIN club_challenge c ON c.id = s.challenge_id JOIN adult_club_membership m ON m.adult_club_id = c.adult_club_id AND ${participant.column} = ? AND m.left_at IS NULL WHERE s.id = ? AND s.moderacion = 'rechazada' AND l.appealed_at IS NULL`).bind(participant.id, stakeId).first<{ id: string }>();
