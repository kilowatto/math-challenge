...
- Der RAND‑Versuch (Pane et al. 2014) fand keinen Effekt im ersten Jahr und etwa 0,21 Standardabweichungen im zweiten Jahr — die Wirksamkeit hing von der Implementierungstreue ab [4].
...
Die ITS‑Forschung teilt sich in zwei häufig miteinander vermischte Stränge: **Model Tracing** (Verfolgung der schrittweisen Lösung eines Schülers anhand von Produktionsregeln — der ursprüngliche Mechanismus von Cognitive Tutor) und **Knowledge Tracing** (Verfolgung der aggregierten Fähigkeitsbeherrschung über Versuche hinweg — Bayesian Knowledge Tracing und Nachfolger) [2]. Die Wirksamkeitsnachweise für das Flaggschiff‑Produkt des Model‑Tracing, Carnegie Learnings Cognitive Tutor/MATHia, sind eindeutig gemischt: Der Review des What Works Clearinghouse von 2016 stuft es als „mixed effects“ in Algebra, „no discernible effects“ im allgemeinen Mathematik‑Erfolg und „potentially negative“ für die Geometry‑Variante ein [3]. Der groß angelegte randomisierte RAND‑Versuch fand keinen Effekt im ersten Jahr und einen bescheidenen Effekt von 0,21 SD im zweiten Jahr, abhängig von der Implementierungstreue [4] — adaptives Tutoring ist nicht automatisch wirksam.

Bayesian Knowledge Tracing (BKT) ist ein verstecktes Markov‑Modell mit vier Parametern (Anfangsbeherrschung, Lernrate, Rate für Raten, Slip) und geschlossenen Aktualisierungsgleichungen [1]. Deep Knowledge Tracing (DKT, Piech et al. 2015) ersetzte dies durch ein LSTM und berichtete große AUC‑Gewinne, doch eine rigorose Replikation (Khajah, Lindsey & Mozer 2016) zeigte, dass der ursprüngliche Vergleich BKT benachteiligte und dass erweitertes BKT den Großteil der Lücke schließt [5][6]. Performance Factors Analysis und das Additive Factors Model bieten eine einfachere logistische Regressionsalternative, die inkrementell passt [7][8].

Das am direktesten anwendbare Vorbild ist **Math Garden (Rekentuin)**, entwickelt an der Universität Amsterdam, kommerzialisiert als Oefenweb/Prowise Learn: ein computer‑adaptives Übungssystem für Grundrechenarten bei Kindern, das die Lernenden‑Fähigkeit und Item‑Schwierigkeit nach jeder Antwort mittels einer Elo‑Variante kombiniert mit der **high‑speed high‑stakes (HSHS) Scoring‑Regel** (Maris & van der Maas, 2010/2012) aktualisiert und dabei sowohl Korrektheit als auch Reaktionszeit bewertet [9][10]. Dies bildet die Grundlage der nachfolgenden konkreten Empfehlung.
...
6. **Ziel‑Erfolgsrate für die Item‑Auswahl: 70–80 %, zentriert bei etwa 75 %**, passend zum validierten .75‑Ziel von Math Garden [9] und der breiteren Literatur zu wünschenswerter Schwierigkeit [11]. Bei der Auswahl des nächsten Items für die Fähigkeit `θ` wählen Sie aus Items, deren Schwierigkeit `β_i` `expected(θ, β_i)` in `[0.70, 0.80]` legt; ziehen Sie unter den 3–5 nächst‑ähnlichen Schwierigkeits‑Items statt immer das einzig nächstliegende, um sichtbar repetitive Sprünge zu vermeiden.
...
11. Wilson et al. (2019). „Die 85‑Prozent‑Regel für optimales Lernen.“ Nature Communications. https://www.nature.com/articles/s41467-019-12552-4  
