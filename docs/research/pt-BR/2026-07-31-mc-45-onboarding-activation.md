...
- **A relação não é uma lei.** Várias fontes documentam casos em que reduzir campos *reduziu* a conversão em 14%, e análises onde dez campos converteram melhor que três [3][5]. A leitura honesta: menos campos ajudam quase sempre, mas é uma hipótese a ser medida, não um axioma.
- **Nielsen Norman Group desaconselha o onboarding, em geral.** Sua recomendação literal é *"evitem criar onboarding de app sempre que possível, e em vez disso gastem esses recursos em tornar a interface mais utilizável"*, por três razões: aumenta o custo de interação, sobrecarrega a memória de trabalho, e a pesquisa mostra que frequentemente não melhora o desempenho real na tarefa [2].
- **O carrossel de cartões é desaconselhado pelo nome.** NN/g aponta explicitamente: faz a interface *parecer mais complexa do que realmente é*, sobrecarrega a memória de trabalho, e sua pesquisa sobre "deck-of-cards tutorials" constatou que **não melhoraram o desempenho na tarefa** [2].
...
- **Não é uma lei.** Fontes documentam casos em que reduzir campos *reduziu* a conversão em 14%, e análises onde dez campos superaram três [3][5]. Leitura honesta: menos campos ajudam quase sempre, mas é uma hipótese a ser medida, não um axioma.
- **Nielsen Norman Group aconselha contra onboarding em geral.** Sua recomendação literal é *"evite criar onboarding de app sempre que possível e, em vez disso, gaste seus recursos tornando a UI mais utilizável"* — porque aumenta o custo de interação, sobrecarrega a memória de trabalho, e pesquisas mostram que frequentemente falha em melhorar o desempenho real da tarefa [2].
- **O formato de carrossel de cartões é desrecomendado pelo nome.** NN/g observa que ele faz as interfaces *parecerem mais complexas do que são*, sobrecarrega a memória de trabalho, e sua pesquisa sobre tutoriais de "deck-of-cards" constatou que **não melhoraram o desempenho da tarefa** [2].
...
### 1. O custo de cada campo de registro

A cifra mais citada e melhor sustentada vem da HubSpot, que estudou formulários de contato de 40.000 clientes: a conversão **subiu quase a metade** ao reduzir de 4 campos para 3 [1]. Um estudo de benchmarks de 2026 traça a curva completa e é a fonte mais útil para orçar campos [5]:
...
Esta é a parte incômoda e a mais valiosa. A recomendação principal da NN/g é que o onboarding **se evite**: *“avoid creating app onboarding whenever possible and instead spend your resources making the UI more usable”* [2]. O raciocínio tem três pilares: aumenta o custo de interação, sobrecarrega a memória de trabalho e a pesquisa mostra que frequentemente **não melhora o desempenho real na tarefa** [2].

A NN/g reconhece exatamente três cenários que justificam telas de onboarding [2]:

1. **Coletar informação indispensável** (o exemplo que dão: criar conta em um app bancário).  
...
**Ajuda contextual: o padrão que a NN/g defende.** Prefere a ajuda no contexto à instrução antecipada, com as pistas surgindo quando a função se torna acionável para o usuário [2].
...
2. Nielsen Norman Group, "Mobile App Onboarding" — https://www.nngroup.com/articles/mobile-app-onboarding/ — fonte primária da posição contra o onboarding, da descoberta sobre tutoriais em formato de baralho de cartas, dos três casos justificados e da regra visual de anotação vs controle.
3. Cobloom, "Form Fields and Conversion Rates: Is Less Really More?" — https://www.cobloom.com/blog/form-fields-and-conversion-rates-is-less-really-more — fonte dos contraexemplos (queda de 14%, dez campos superando a três).
...
**Advertência de método e qualidade das fontes.** Apenas a fonte [2] é pesquisa primária de uma organização de UX independente. As fontes [1], [3], [4] e [5] são publicações da indústria de marketing e de fornecedores de ferramentas de formulários, com interesse comercial no tema que medem; a métrica da HubSpot é citada de segunda mão porque o estudo original não foi recuperável diretamente nesta sessão. As métricas de conversão do §1 devem ser tratadas como **ordem de magnitude direcional**, não como constantes. A métrica de "68% mais engajamento" que circula atribuída à NN/g **não pôde ser verificada e não é usada** (§4).
