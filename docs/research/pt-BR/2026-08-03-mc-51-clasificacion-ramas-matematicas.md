# Classificação dos ramos da matemática e sua estrutura de pré-requisitos — do MSC 2020 aos currículos universitários e aos currículos escolares do mundo

> Pesquisa Math Challenge — 2026-08-03 — tópico 51

## Resumo executivo (PT)

- Existem três classificações vivas dos ramos da matemática, e são
  distintas de propósito: **MSC 2020** (63 áreas de dois dígitos, a
  citável por padrão — licença aberta, manutenção dual AMS/zbMATH, revisão
  decenal) [1][2], **arXiv math** (32 categorias, a que reflete a prática
  atual de pesquisa) [3], e as **seções do ICM** (20 em 2022, revisadas a
  cada quatro anos pelo Structure Committee) [4][5]. Para o nível escolar
  não existe taxonomia de ramos: o que há é PISA (4 categorias de conteúdo
  de avaliação) e ISCED-F da UNESCO (0541 Mathematics / 0542 Statistics)
  [6][7].
- A **estrutura de pré-requisitos escolar é universal**: contagem →
  adição/subtração → multiplicação/divisão → frações (exige divisão) →
  decimais → porcentagem (exige frações/100) → razão e proporção (exige
  ...
  **simultaneamente** geometria e funções) → cálculo (exige funções).
  ...
  [8]-[17].
- **O que muda por país não é a ordem, mas o empacotamento**: EUA permitem
  Algebra I → Geometry → Algebra II ou integrado; Espanha bifurca no 4º
  ESO (A/B); França especializa em première; Singapura faz streaming
  (Standard/Foundation); o O-level base de Singapura e o GCSE **não
  incluem cálculo**.
- Na universidade, a cadeia dominante verificada em 10 instituições (MIT,
  ...
  Sorbonne) [18]-[27]: **álgebra linear vem depois ou em paralelo ao
  ...
  da medida); o **curso-ponte de demonstração** existe em toda parte com
  três desenhos (curso explícito, integrado desde o dia 1, trilha
  acelerada); a análise real exige só cálculo, mas tudo o que segue
  (análise funcional, variedades, EDPs) exige **análise real ∧ álgebra
  linear** ao mesmo tempo.
- Consequência para o Math Challenge: a escada N1-N12 deve levar
  **códigos de ramo** e **arestas de pré-requisito** separadas do nível de
  ...
  nível» (D-122) não pode garantir cobertura por ramo — que é justamente o
  que este documento habilita medir.
- Advertência de design: o exemplo ingênuo «álgebra exige geometria
  básica» **é falso em todos os sistemas verificados** — a geometria corre
  em espiral paralela, não como pré-requisito de álgebra. As dependências
  duras reais são: frações exigem divisão; porcentagem exige frações;
  razão exige frações; trigonometria exige geometria + funções; cálculo
  exige funções; análise real exige cálculo; análise
  funcional/EDPs/probabilidade teórica exigem análise real + álgebra
  linear (+ medida).
...
A Mathematics Subject Classification, revisão 2020, publicada
conjuntamente por Mathematical Reviews (AMS) e zbMATH sob licença
CC-BY-NC-SA [1][2]: **63 áreas de dois dígitos, 529 de três, 6.022 de
cinco** [2]. Cada item classificado leva exatamente uma classe primária e
zero ou mais secundárias [2].

As 63 áreas de dois dígitos, do PDF oficial baixado [1]:
...
Mudanças 2010 → 2020: nenhuma em dois dígitos («não se criaram novos
campos»); nove classes novas de três dígitos (18M, 18N, 53E, 57K, 57Z,
60L, 62R, 68V, 82M); 113 classes de cinco retiradas e 486 novas; hífens
uniformizados (`-08` métodos computacionais, `-10` modelagem, `-11` dados
de pesquisa) [2]. O artigo oficial da revisão é Dunne & Hulek, EMS
Newsletter [28].

**Nota estrutural relevante para nós:** o MSC não é uma árvore, é um DAG —
as classes levam referências cruzadas explícitas (`{For differential
topology, see 57Rxx}`) [1]. Uma classificação de ramos que não admita
links cruzados está mal desenhada desde o primeiro dia.
...
(diferencial), DS (dinâmicos), FA (funcional), GM, GN (topologia geral),
GR (grupos), GT (geométrica), HO, IT, KT, LO (lógica), MG (métrica), MP
(física matemática), NA (numérica), NT (números), OA (operadores), OC
(otimização e controle), PR (probabilidade), QA (quântica), RA (anéis), RT
(representação), SG (simplética), SP (espectral), ST (estatística).
Reflete fronteiras vivas que o MSC reparte (K-theory, simplética, quântica
têm categoria própria), mas não tem processo formal de revisão publicado.

### 1.3 Seções do ICM — a visão comunitária quadrienal

As 20 seções do ICM 2022, das atas oficiais EMS Press [4]: Logic; Algebra;
Number Theory; Algebraic and Complex Geometry; Geometry; Topology; Lie
Theory; Analysis; Dynamics; PDEs; Mathematical Physics; Probability;
...
History. **Não são fixas**: o Structure Committee da IMU as revisa a cada
quatro anos (primeiro presidente: Terence Tao; desde agosto de 2025:
Martin Hairer) [5].

### 1.4 O nível escolar: não há taxonomia de ramos, e há que dizê-lo
...
- **PISA 2022** classifica o conteúdo de *avaliação* em quatro categorias:
  ...
  shape (~¼ de itens cada uma) — não são ramos, são lentes de medição [6].
- **ISCED-F 2013 (UNESCO)** classifica programas educativos: campo `054` →
  **0541 Mathematics** / **0542 Statistics** [7].
- Os currículos nacionais (§3) organizam por domínios ou fios temáticos,
  não por ramos de pesquisa. Qualquer catálogo que pretenda mapear
  «ramos» a idades escolares está construindo algo novo — como este.
...
variam o empacotamento e o ano.

### 2.1 Os nove elos, com sua dependência dura

1. **Contagem e cardinalidade → adição e subtração.** Universal (CCSS K-1,
   NC Y1, Singapura P1, México fase 3, Baviera primária).
2. **Multiplicação e divisão** com o sistema posicional em paralelo. A
   ...
   frações, razão, porcentagem e média.
3. **Frações** (CCSS 3.NF, NC Y3-Y5, Singapura P2-P5, BNCC Números).
   **Exige divisão consolidada.** Os três sistemas mais detalhados tratam
   frações/decimais/porcentagens como «formas do mesmo número» (literal em
   NC Y4-Y5).
4. **Decimais → porcentagem.** **Porcentagem exige frações com denominador
   100** — é uma razão por cem (CCSS 6.RP.3c, NC Y5).
5. **Razão e proporcionalidade** — a ponte para o ensino médio (CCSS 6.RP,
   NC Y6, México «variação», a big idea «Proportionality» de Singapura, o
   «taux d'évolution» francês). **Exige multiplicação, divisão e frações
   ao mesmo tempo** — é a primeira dependência múltipla real.
6. **Inteiros com sinal e pré-álgebra → equações lineares → funções
   lineares** (CCSS 7.NS → 8.EE → 8.F; Baviera M5 ℤ → M8; França seconde).
7. **Quadráticas e polinômios → exponenciais e logaritmos** (França 1ʳᵉ:
...
9. **Cálculo** (derivada → integral). **Exige funções e limites
   intuitivos.** É o conteúdo de fechamento — com duas exceções de peso:
   **o O-level base de Singapura e o GCSE inglês NÃO incluem cálculo**
   [16][10]; entra aos 16-17 na França e na Espanha [14][13], aos 17-18 em
...
ensino fundamental em **todos** os sistemas verificados [8]-[17].
**Geometria não é pré-requisito de álgebra em nenhum**: são fios paralelos
que se encontram tarde (na geometria analítica e na trigonometria). A
afirmação «você não pode fazer álgebra sem geometria básica» é falsa como
pré-requisito; a verdade estrutural é que as duas espirais se *cruzam* em
pontos concretos: coordenadas (álgebra × geometria), inclinação (razão ×
funções), trigonometria (geometria × funções), vetores (álgebra linear ×
geometria).

### 2.3 O que varia por país (e por que importa a um produto em 7 locales)

- **Empacotamento do ensino médio**: EUA permitem Algebra I → Geometry →
  Algebra II *ou* integrado — os estados decidem (Appendix A do CCSS) [8];
  Espanha integra e bifurca no 4º ESO (Matemáticas A acadêmicas / B
  aplicadas) e por modalidade no Bachillerato com precedência formal
  1º→2º [13]; França: tronco comum em seconde + especialidade [14];
  Alemanha: integrado por Jahrgangsstufe [15]; Singapura: streaming
  Standard/Foundation em P5 e Express/NA/NT depois [16].
- **Quando entra a demonstração formal**: França e o IB (Proofs, só HL) a
  tornam explícita [14][12]; EUA a concentram no curso de Geometry; o NC
  inglês a dilui em «reason mathematically» [9].
- **México em transição**: a MCCEMS (Acuerdo 09/08/23) reorganiza o ensino
  médio superior em progressões transversais de «Pensamento Matemático» em
  vez das matérias clássicas Álgebra → Geometria e Trigonometria → Cálculo
  [11]. Um produto que fale de «álgebra de ensino médio» fala com o
  sistema velho.

## 3. A estrutura de pré-requisitos universitária: dez instituições verificadas

Fontes primárias baixadas: catálogos e currículos oficiais de MIT [18],
...
**Padrão 1 — Álgebra linear vem depois ou em paralelo ao multivariável,
nunca antes do cálculo de uma variável.** MIT: `18.06` exige `18.02` [18].
Harvard: `21b` (linear + EDOs) após `21a` [19]. Berkeley: `54` após `52`,
junto a `53` [25]. Princeton: `202` após `104`, intercambiável com `201`
[24]. Stanford: integrado (`Math 51` é linear + multivariável) [20]. Os
sistemas europeus de ano fechado (Cambridge, Oxford, ETH, UNAM) a colocam
no primeiro ano **em paralelo** com a análise, porque todo o ano 1 é
núcleo [21][22][23][26].
...
medida, **depois da análise real**: ETH declara `Analysis III
...
**Padrão 3 — O curso-ponte de demonstração existe em toda parte, com três
...
trilha acelerada paralela: Harvard `25/55`, Stanford `60CM/DM`, Princeton
`215-217`. **Nenhum sistema lança um estudante a cursos de demonstração
sem ponte.**

**Padrão 4 — A análise real exige só cálculo; todo o resto exige ambos.**
...
**Padrão 5 — Dois níveis de álgebra linear** (computacional → teórica):
MIT `18.06` → `18.700`; Berkeley `54` → `110`; Stanford `51` → `113`;
Cambridge `Vectors and Matrices` → `Linear Algebra`. A segunda passagem é
a que alimenta álgebra abstrata, representações e geometria.
...
As dependências **duras de inscrição** são fenômeno estadunidense
...
Síntese de §1-§3 numa classificação operativa. Cada ramo leva seu código
MSC 2020, sua posição escolar (se existe) e sua porta de pré-requisitos. É
a classificação que o catálogo de desafios
...
| `DECI` | Decimais e porcentagem | frações | N6 |
...
| Código | Ramo | Porta (exige) | Nível N |
|---|---|---|---|
| `PREA` | Pré-álgebra e sequências | aritmética | N7 |
| `ALGE` | Álgebra (equações, sistemas, polinômios) | pré-álgebra | N8 |
...
| `GEOA` | Geometria analítica (coordenadas, inclinação, distância) | álgebra | N8 |
...
| `DEMO` | Demonstração (validação, detectar o erro) | ponte — a «trilha Lean 4» de D-124 | N11-N12 |

**Regras do mapa:** (1) o nível N é de **dificuldade**, o ramo é de
**família** — movem-se separadamente (D-017 já o diz para tema visual e
nível; aqui é o mesmo princípio uma camada abaixo). (2) As portas são a
metade verificada deste documento (§2-§3), não critério: cada uma cita seu
sistema. (3) O mapa é um DAG como o MSC, não uma árvore: `TRIG` tem duas
portas, `VCOM` e `EDOS` duas cada uma.
...
1. **O banco ganha um campo `rama`** (os códigos de §4), separado de
   ...
   cumprir-se com seis desafios do mesmo ramo e ninguém notaria. Com ele,
   o auditor do piso (`piso-seis-retos.mjs`) pode exigir cobertura: ≥3
   ramos distintos por nível.
2. **As arestas de pré-requisito do adaptativo** (F4) devem vir de §4, não
   ser inventadas: `skill_state` agenda nós de habilidade, e a pergunta
   «pode ver frações?» se responde com a aresta `FRAC ← división`. A tabela
   de §4 é a primeira fonte verificada do projeto para essas arestas.
3. **O catálogo dos 54 desafios fica medido pela primeira vez.** Cobertura
   ...
   ramo explícito (está repartido), e `DIVI` solto fora de N4**. As
   lacunas são compatíveis com D-124 (os formatos auto-avaliáveis mandam),
   mas agora são visíveis e decidíveis, não invisíveis.
4. **A «ponte de demonstração» tem que existir como trilha**, não como
   esperança: os dez sistemas universitários a têm, e nossa versão é a
   trilha Lean 4 (D-124) mais os formatos de `mc-12` (detectar o erro,
   ordenação de passos) já presentes em N11-N12 do catálogo.
5. **Geometria corre em espiral, não se «passa» uma vez**: o mapa a coloca
...
6. **México e Singapura introduzem álgebra cedo** (variação/big ideas);
   EUA esperam até o 6º ano. Para os 7 locales, o nível N de um desafio é
   neutro ao país (D-017/mc-15) — e esta pesquisa é a evidência de por que
   deve continuar sendo: o conteúdo por idade varia; a estrutura de
   dependências, não.
7. **A probabilidade se ensina duas vezes** (cedo com cálculo, rigorosa
   com medida). Nosso `PROB` de N9 é a primeira; a segunda não é
   auto-avaliável com os formatos atuais e fica explicitamente fora.
8. **MSC 97 (Mathematics education) e a seção 19 do ICM** são a ponte
   oficial entre pesquisa e ensino — este documento vive aí, e a
   classificação de §4 deve ser revisada quando o MSC 2030 sair (ciclo
   decenal declarado [2]).

## 6. Perguntas abertas para o dono — RESOLVIDAS (2026-08-03)

| # | Pergunta | Resposta | Decisão |
|---|---|---|---|
| 1 | Códigos próprios ou MSC? | **MSC de 2 dígitos, com nome em linguagem de pessoas autorado por locale** (resposta personalizada do dono: rigor do padrão, cara de pessoa) | D-128 |
| 2 | Cobertura do auditor do piso? | ≥3 ramos MSC distintos por nível, além dos 6 desafios | D-129 |
| 3 | `VCOM`/`EDOS`? | Entram como níveis futuros — `mc-12` confirma que são auto-avaliáveis | D-130 |
| 4 | As espirais como arestas? | Fracas (recomendam, não bloqueiam); duras só as verificadas | D-131 |
| 5 | A ponte de demonstração? | Trilha transversal visível no mapa, não um nível a mais | D-132 |
| 6 | Quem a mantém atualizada? | Entradas datadas em `dudas.md`: 2027 (currículos) e 2030 (MSC) | D-133 |
...
1. **MSC 2020, PDF oficial completo** — zbMATH/Mathematical Reviews —
   https://zbmath.org/static/msc2020.pdf — baixado; fonte das 63 áreas e
   das referências cruzadas.
2. **MSC 2020 portal e regras de uso** — https://msc2020.org/ e
   https://mathscinet.ams.org/msc/msc2020.html — baixados; contagens
...
3. **arXiv category taxonomy** — https://arxiv.org/category_taxonomy —
   baixado; as 32 categorias math.* com descrições.
4. **Atas do ICM 2022, sumário** — EMS Press —
   https://www.proceedings.com/content/074/074997webtoc.pdf — baixado; as
   20 seções.
5. **IMU — ICM Structure Committee** — https://www.mathunion.org/icm/icm-2022
   e https://www.mathunion.org/icm/icm-2026 — baixados; a revisão
   quadrienal de seções. As seções do ICM 2026 não puderam ser baixadas
...
7. **UNESCO ISCED-F 2013** — uis.unesco.org (PDF oficial não baixável
...
8. **Common Core State Standards** — https://www.thecorestandards.org/Math/
   e páginas de domínio por ano (K, 3.NF, 6.RP, 8.F, HSA) — baixadas.
9. **National Curriculum in England: Mathematics programmes of study** —
   gov.uk — baixado (Y1-Y5 literal; KS3/KS4 truncado). GCSE: página índice
   baixada, conteúdo corroborado. A-level: 404, **não verificado**.
10. **IB — Mathematics in the DP** — https://www.ibo.org/programmes/diploma-programme/curriculum/mathematics/
    — baixado (4 cursos, 5 temas, Proofs e Vectors só HL).
11. **México SEP** — Campo formativo Saberes y Pensamiento Científico (PDF
    ...
    (gob.mx) — baixados. A nota do DOF do Acuerdo 09/08/23 falhou por
    rede.
12. **IB Group 5 subjects** — Wikipedia — baixada (fonte secundária,
    cruzada com [10]).
13. **Espanha LOMLOE** — RD 217/2022 (ESO) e RD 243/2022 (Bachillerato),
    BOE — baixados (a precedência 1º→2º do art. 21,2, literal; anexos de
    conteúdo truncados).
14. **França, programme de spécialité Mathématiques de première** (PDF
    oficial) e BO maths intégré à l'enseignement scientifique —
    education.gouv.fr — baixados completos. Seconde e terminale: não
    baixados.
15. **Alemanha, LehrplanPLUS Bayern Gymnasium** (M5, M10) e KMK
    Bildungsstandards — baixados. É um Land de 16: não extrapolar para
    toda a Alemanha sem cuidado.
16. **Singapura** — MOE Primary Mathematics Syllabus 2021 (PDF) e SEAB
    O-Level Mathematics 4052 (PDF) — baixados. Nota: a abordagem CPA não
    aparece com esse nome no syllabus oficial (vive nos materiais
    didáticos) — dito para que ninguém a cite do documento errado.
17. **Brasil BNCC** — portal oficial é aplicação JS (não baixável);
    estrutura corroborada via Wikipedia (baixada). As 5 unidades temáticas
    (Números; Álgebra; Geometria; Grandezas e medidas; Probabilidade e
    estatística). Detalhe por ano: **pendente de verificação fina**.
18. **MIT** — catálogo Course 18 (student.mit.edu/catalog/m18a.html,
    catalog.mit.edu/subjects/18/) e roadmaps do departamento — baixados;
    os pré-requisitos literais citados em §3.
19. **Harvard** — Courses in Mathematics (PDF do departamento) — baixado;
    os cursos-ponte e «Math 123 cannot be taken before Math 122».
20. **Stanford** — bulletin.stanford.edu/programs/MATH-BS — baixado.
21. **Cambridge** — Guide to Courses in Part IA e IB 2024-25 (PDFs) —
    baixados; as dependências advisorias («builds on», «essential for»).
22. **Oxford** — prospectus oficial do Mathematical Institute — baixado; a
    tabela ano a ano.
23. **ETH Zürich** — Wegleitung Bachelor Mathematik 2024 (PDF, em alemão)
    — baixado; Masstheorie como base de probabilidade/funcional/EDPs,
    textual.
24. **Princeton** — Standard Math Sequences e Overview of Lower Division
    (math.princeton.edu) — baixados; a via dupla example/proof-based.
25. **UC Berkeley** — Course Requirements: Pure Mathematics
    (math.berkeley.edu) — baixado.
26. **UNAM, Facultad de Ciencias** — ficha da licenciatura, currículo
    (oferta.unam.mx PDF) e estrutura DGAE-SIAE — baixados; a «seriación
    indicativa» e as cadeias numeradas.
27. **Sorbonne Université** — Onisep, licence mention mathématiques —
    ...
    portais L1 e as bifurcações L2/L3. **A maquette curso a curso não
    ficou verificada.**
...
circula por aí não pôde ser certificado; A-level, BNCC por ano, seconde/
terminale e a maquette da Sorbonne ficam marcados como não baixados. Todo
o resto tem URL verificada acima.
