...
Math Challenge é uma aplicação de prática de matemática, concebida como PWA‑first, construída inteiramente na Cloudflare. A arquitetura proposta utiliza **Workers + Astro** para o frontend/BFF, **D1** para dados relacionais (contas, conteúdo, adesões), **Durable Objects with SQLite storage** para estado ao vivo de baixa cardinalidade (uma liga de ~30, uma turma, a sessão de uma criança), **Analytics Engine** para telemetria de tentativas de alto volume (não D1 — D1 esgota o armazenamento primeiro), **KV** para instantâneos de classificação global/banda de notas recalculados periodicamente, **R2** para media e ficheiro frio, **Queues + Workflows** para pontuação assíncrona e geração de explicações por IA, **Vectorize + Workers AI** para RAG multilingue sobre o banco de dicas, e **AI Gateway** na frente da Claude API para armazenar em cache, limitar a taxa e impor um limite de despesa ao tutor “Larry” com encaminhamento de modelo. O limite que atingimos primeiro não é a computação — é o teto de armazenamento do D1 (10 GB por base de dados no plano pago) se as tentativas brutas forem armazenadas lá. É por isso que as tentativas brutas vão para o Analytics Engine, e não para o D1.
...
| `math-challenge-db` | D1 database | Sistema de registo: utilizadores, crianças, salas de aula, ligas, metadados de conteúdo, consentimento | Registo maestro: utilizadores, niños, salones, ligas, metadatos de contenido, consentimiento | `DB` |
...
| `math-challenge-ratelimiter-do` | Durable Object class (SQLite) | Limitação de taxa fragmentada (tentativas de login, chamadas ao tutor, registo) | Limitación de tasa fragmentada (inicios de sesión, llamadas al tutor, registo) | `RATE_LIMITER_DO` |
...
| `math-challenge-turnstile-signup` | Turnstile widget | Defesa contra bots em formulários de registo/início de sessão | Defensa contra bots en formularios de registo/inicio de sesión | (site key/secret via env) |
